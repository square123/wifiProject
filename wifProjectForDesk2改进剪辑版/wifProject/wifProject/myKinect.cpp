...
float Kinect::detPeopleDepth()//直接用关节点作为深的输出，减少计算,似乎通过关节点好像不太可靠，可能是因为点数太少了，所以不可靠，不能获得稳定的输出
{
...
////int Kinect::detPeopleDepth2()   //已废弃不用 采用的是最近人的深度
////{
////	UINT8 bodyInt[6]={15,55,95,135,175,215};//用于区别6个人
////	bodyIndexProcess();
////	int Temp=7000;
...
////	//加一个框 框应该是一个小矩形Mat window=Mat::zeros(DepthHeight, DepthWidth, CV_8UC3 );//先建立一个窗
////	//将线修改成矩形 
////	rectangle(window,rec_tl,rec_br,Scalar(255,255,255),-1,8);
////	Mat deWindow;
////	cvtColor(window,deWindow,CV_BGR2GRAY);
////	
////	//完成与深度的与操作
////	//imshow("xiao",deWindow);//
////	Mat bodyDepth0,bodyDepth1,bodyDepth2,bodyDepth3,bodyDepth4,bodyDepth5;
////	depthMat16.copyTo(bodyDepth0,body0&deWindow);
////	depthMat16.copyTo(bodyDepth1,body1&deWindow);
////	depthMat16.copyTo(bodyDepth2,body2&deWindow);
////	depthMat16.copyTo(bodyDepth3,body3&deWindow);
////	depthMat16.copyTo(bodyDepth4,body4&deWindow);
////	depthMat16.copyTo(bodyDepth5,body5&deWindow);
////    int bodyDepthMean[6];//用来返回每个人的深度
////	bodyDepthMean[0]=avgMat(bodyDepth0,areaMatOri(Mat(body0&deWindow)));  
////	bodyDepthMean[1]=avgMat(bodyDepth1,areaMatOri(Mat(body1&deWindow)));
////	bodyDepthMean[2]=avgMat(bodyDepth2,areaMatOri(Mat(body2&deWindow)));
////	bodyDepthMean[3]=avgMat(bodyDepth3,areaMatOri(Mat(body3&deWindow)));
////	bodyDepthMean[4]=avgMat(bodyDepth4,areaMatOri(Mat(body4&deWindow)));
////	bodyDepthMean[5]=avgMat(bodyDepth5,areaMatOri(Mat(body5&deWindow)));
////	for(int i=0;i<BODY_COUNT;i++)
////	{
