...
void overlapPro::timeRawProcess()
{
	...
	for (int i=0;i<count_num;i++)//每个count
	{
		...
		if (bwMixIndex[second][i]!=0)//如果空不操作
		{
			...
			outfile<<timeFixed<<","<<i<<","<<xAve<<","<<yAve<<endl;
		}
	}
	//清空操作
...
//也是为了克服遮挡问题
void overlapPro::kalmanfilter()
{
	
}

//轨迹变换矩阵（要根据实地场合去计算）暂时先不考虑 或者提前做好查表工作
void overlapPro::pointTransform(Point &src,Point &dst)
{
...
void overlapPro::process(Mat &src)
{
	...
	selectArea(src,selectedMat,180,4000);//通过函数对一些连通域小的进行筛选  以后可以根据实地判断筛选 针对稀疏人物目标的跟踪
