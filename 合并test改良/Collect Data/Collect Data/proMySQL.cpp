...
void proMySQL::creBigTable(const char *tableName)
{
	...
	sqlstr +=  
		"民族 VARCHAR(10) CHARACTER SET gb2312 COLLATE gb2312_chinese_ci NULL COMMENT '民族',";
	sqlstr +=  
	...
	sqlstr +=  
		"身份证照片 VARCHAR(100) CHARACTER SET gb2312 COLLATE gb2312_chinese_ci NULL COMMENT '照片',";//照片存储路径
	sqlstr +=  
		"第一Mac地址 varchar(20) null COMMENT '第一Mac地址',";  //Mac1
	sqlstr +=  
		"第二Mac地址 varchar(20) null COMMENT '第二Mac地址',";  //Mac2
...
void proMySQL::insertBigTabData(const char *tableName,string &sTime,vector<string> &mac,const char *macSavePath,const char *perform,char *idData)
{
	string sqlstr="INSERT INTO ";   //还需要测试下
	sqlstr+=tableName;
	sqlstr+=" VALUES(";
	...
	sqlstr+=", '";
	sqlstr+=string(idData+292).size()!=0?string(idData+292):"";
	sqlstr+="'";
