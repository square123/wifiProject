...
public:
	...
	map<string,vector<int>> selMap,selMapDing;//新的存储格式   //针对数据库可以在以后只存储最可能的前5个数据量，便于保存
	struct mapUsed  //在处理时要使用的格式
	{
		string macName;
		int num;
		int avgRssi;
	...
	void  wifiProcessed2(map<string,vector<int>> &ss,vector<mapUsed> &,ofstream &raw);//新的方法 完成的是排序后的输出
	void reSelMacRssi();
